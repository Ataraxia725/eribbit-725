...
export default {
  ...
  setup (props) {
    ...
    const bgPosition = reactive({ backgroundPositionX: 0, backgroundPisitionY: 0 })
    watch([elementX, elementY, isOutside], () => {
      ...
      bgPosition.backgroundPisitionY = -position.top * 2 + 'px'
      position.left = position.left + 'px'
      position.top = position.top + 'px'
      show.value = !isOutside.value
    })
    //   const { currIndex, togleImg } = useToggleImg()
